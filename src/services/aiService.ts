...
class AIService {
      ...
      try {
        ...
        if (result.suggestedCategory) {
          const matchedCategory = kb.categories.find(
            (c) =>
              c.id === result.suggestedCategory?.id ||
              c.name.toLowerCase() ===
                result.suggestedCategory?.name.toLowerCase(),
          );
          if (matchedCategory) {
        ...
        if (error instanceof Error) {
          ...
          if (!isTimeoutOrAbort) {
            ...
            console.warn(
              "⏱️ AI request timed out - model may still be processing",
            );
            // Provide a helpful timeout message
            throw new Error(
              "The AI is taking longer than expected. Please try again - your request may have been too complex.",
            );
          }
        }
  ...
  async generateSummary(
    ...
    if (!kb) {
      return this.mockGenerateSummary(
        messages,
        extractedFields,
        category,
        priority,
    ...
    if (this.config.useGroq && this.groqAvailable) {
      try {
        ...
        if (result.seriousnessLevel?.priority_level === "critical")
          tags.push("critical");
        if (result.seriousnessLevel?.priority_level === "high")
          tags.push("high-priority");
        if (result.issueTopics?.issue_category)
          tags.push(result.issueTopics.issue_category);

        // Include caller name in notes if extracted
        let notes = result.notes;
        if (
          result.callerName &&
          !notes.toLowerCase().includes(result.callerName.toLowerCase())
        ) {
        ...
        if (result.sentimentCards) {
          console.log(
            `   📊 Generated ${result.sentimentCards.length} sentiment cards`,
          );
        }
  ...
  private async smartMockResponse(
    ...
    throw new Error(
      "AI model unavailable - all responses must be generated by the cloudflare model. Ensure your cloudflare tunnel is running.",
    );
  }
  ...
  private async mockGenerateSummary(
    ...
    if (
      ...
      purpose?.toLowerCase().includes("schedule")
    ) {
      actionItems.push({
  ...
  async predictCategory(
    ...
    if (allText.includes("appointment") || allText.includes("schedule")) {
      return {
        category:
          categories.find((c) => c.id === "appointment") || categories[0],
        confidence: 0.92,
