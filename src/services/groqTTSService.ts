...
class GroqTTSService {
  ...
  async initialize(): Promise<boolean> {
    ...
    if (typeof window !== "undefined" && typeof navigator !== "undefined") {
      ...
      if (this.isMobile) {
        log.debug(
          `📱 Mobile device detected (iOS: ${this.isIOS}, Android: ${this.isAndroid}), using mobile-specific audio handling`,
        );
      }
    }
  ...
  async unlockAudio(): Promise<void> {
    ...
    try {
      ...
      if (this.isMobile && !this.persistentMobileAudio) {
        log.debug(
          "📱 Creating persistent mobile audio element SYNCHRONOUSLY...",
        );
  ...
  private getAudioElement(): HTMLAudioElement {
    ...
    if (this.isMobile) {
      if (this.persistentMobileAudio) {
        log.debug(
          "📱 Reusing persistent mobile audio element (maintains unlock state)",
        );
      ...
      log.warn(
        "⚠️ Creating fresh audio element for mobile - may not play without user gesture",
      );
      const audio = new Audio();
      audio.setAttribute("playsinline", "true");
      audio.setAttribute("webkit-playsinline", "true");
    ...
    return new Promise((resolve, reject) => {
      ...
      (async () => {
        try {
          ...
          const processedText = directionPrefix
            ? `${directionPrefix} ${formattedText}`
            : formattedText;

          log.debug(
            `🎤 Synthesizing speech with Groq TTS (voice: ${voice}, direction: ${vocalDirection}, iOS: ${this.isIOS})`,
          );

          // Use MP3 format for better iOS Safari compatibility
          const audioBlob = await proxyBlob(
            ProxyRoutes.TTS,
            {
          ...
          try {
            ...
            if (this.isIOS && this.audioContext) {
              log.debug(
                "📱 iOS: Using Web Audio API for playback (more reliable)...",
              );
              ...
              try {
                ...
                log.warn(
                  "⚠️ iOS Web Audio failed, falling back to HTMLAudioElement:",
                  webAudioError,
                );
            ...
            if (this.isMobile) {
              log.debug(
                "📱 Mobile: Setting src and playing with HTMLAudioElement...",
              );
            ...
            if (
              ...
              log.warn(
                "⚠️ HTMLAudioElement blocked - trying Web Audio API fallback...",
              );
              ...
              try {
                ...
                log.error(
                  "❌ Web Audio API fallback also failed:",
                  webAudioError,
                );
              }
            }
