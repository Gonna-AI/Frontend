    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (response.status === 429 && attempt < maxRetries) {
          ...
          console.warn(
            `[proxyClient] Rate limited on /${route}, retrying in ${retryAfter}s (attempt ${attempt + 1}/${maxRetries})`,
          );
        ...
        if (isRetryableStatus(response.status) && attempt < maxRetries) {
          ...
          console.warn(
            `[proxyClient] Server error ${response.status} on /${route}, retrying in ${delay}ms (attempt ${attempt + 1}/${maxRetries})`,
          );
          await sleep(delay);
          continue;
        }
        ...
        if (attempt < maxRetries) {
          const delay = BASE_DELAY_MS * Math.pow(2, attempt);
          console.warn(
            `[proxyClient] Network error on /${route}, retrying in ${delay}ms (attempt ${attempt + 1}/${maxRetries}):`,
            err,
          );
          await sleep(delay);
          continue;
...
export async function proxyJSON<T = unknown>(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.error || `Proxy request failed: ${response.status}`,
    );
  }
