...
export function AccessCodeProvider({
    ...
    const checkAccess = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/api-access`,
          {
            headers: {
  ...
  const validateCode = useCallback(async (code: string): Promise<boolean> => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/api-access/validate`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${session.access_token}`,
  ...
  return (
    <AccessCodeContext.Provider
      value={{ hasAccess, isLoading, error, validateCode, clearError }}
    >
      {children}
    </AccessCodeContext.Provider>
  );
