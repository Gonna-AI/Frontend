  ...
  const tagCounts = allTags.reduce(
    (acc, tag) => {
      if (tag === "All") {
        ...
        acc[tag] = allBlogs.filter((blog) => blog.tags?.includes(tag)).length;
      }
      return acc;
    },
          ...
          <Suspense
            fallback={
              <div className="text-white px-6">
                {t("blogPage.loadingArticles")}
              </div>
            }
          >
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative auto-rows-fr px-6 lg:px-0">
