...
const AdminDashboard = () => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      setApiStats({
        ...
        clients_verified_today:
          response.data.statistics.clients_verified_today || 0,
      });
    } catch (error) {
  ...
  const fetchBlockchainInfo = async (documentId: string) => {
    try {
      ...
      const selectedDoc = selectedDocument?.documents?.find(
  ...
  const renderBlockchainInfo = () => (
    <div
      className={`${theme.statsBg} border ${theme.statsBorder} rounded-lg p-4`}
    >
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-lg font-medium">
          {t("documents.blockchain.details")}
        </h3>
        <button
          onClick={() => {
      ...
      {blockchainInfo ? (
        <div className="space-y-3 text-sm">
          ...
          <div className="flex justify-between items-center">
            ...
            >
              {blockchainInfo.isVerified
                ? t("documents.blockchain.verified")
                : t("documents.blockchain.pending")}
            </span>
          </div>

          <div className="flex justify-between items-start gap-4">
            <span className={theme.secondaryText}>
              {t("documents.blockchain.uploader")}
            </span>
            <span className="font-mono text-right break-all">
              {blockchainInfo.uploader}
            </span>
          </div>

          <div className="flex justify-between items-center">
            <span className={theme.secondaryText}>
              {t("documents.blockchain.uploadTime")}
            </span>
            <span>
              {new Date(blockchainInfo.timestamp * 1000).toLocaleString()}
            </span>
          </div>
          ...
          {blockchainInfo.isVerified && (
            <>
              <div className="flex justify-between items-start gap-4">
                <span className={theme.secondaryText}>
                  {t("documents.blockchain.verifier")}
                </span>
                <span className="font-mono text-right break-all">
                  {blockchainInfo.verifier}
                </span>
              </div>

              <div className="flex justify-between items-center">
                <span className={theme.secondaryText}>
                  {t("documents.blockchain.verificationTime")}
                </span>
                <span>
                  {new Date(
                    blockchainInfo.verificationTime * 1000,
                  ).toLocaleString()}
                </span>
              </div>
            </>
      ...
      if (clientDocs && clientDocs.documents) {
        ...
        await Promise.all(
          clientDocs.documents.map(async (doc) => {
            const blockchainResponse = await adminApi.verifyBlockchain(
              doc.document_id,
            );
        ...
        setDocuments((prevDocs) =>
          prevDocs.map((doc) =>
            doc.ticketId === ticketId ? { ...doc, status: "verified" } : doc,
          ),
        );
  ...
  const handleReject = async (ticketId: string) => {
    try {
      ...
      if (clientDocs && clientDocs.documents) {
        ...
        setDocuments((prevDocs) =>
          prevDocs.map((doc) =>
            doc.ticketId === ticketId ? { ...doc, status: "rejected" } : doc,
          ),
        );
  ...
  const handleExportReports = () => {
    try {
      ...
      link.setAttribute(
        "download",
        `documents_report_${new Date().toISOString().split("T")[0]}.csv`,
      );
      document.body.appendChild(link);
      link.click();
      link.remove();
  ...
  const renderStatistics = () => {
    ...
    const totalClients =
      apiStats.pending_review + apiStats.clients_verified_today;
      ...
      >
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div
              className={`p-3 ${theme.statsBg} border ${theme.statsBorder} rounded-lg`}
            >
              <div className={`text-2xl font-bold ${theme.text}`}>
                {apiStats.pending_review}
              </div>
            ...
            >
              <div className={`text-2xl font-bold ${theme.text}`}>
                {apiStats.clients_verified_today}
              </div>
              <div className={theme.statsText}>
                {t("documents.stats.verified")}
              </div>
            </div>
          </div>
          <div
          ...
          >
            <div className={theme.statsText}>
              {t("documents.stats.progress")}
            </div>
            <div
              className={`h-2 ${isDarkMode ? "bg-gray-700" : "bg-gray-200"} rounded-full overflow-hidden mt-2`}
            >
              <div
            ...
            <div className="flex justify-between mt-1">
              <div className={theme.statsText}>
                {completionPercentage}% {t("documents.stats.complete")}
              </div>
              <div className={theme.statsText}>
                {apiStats.clients_verified_today} / {totalClients}{" "}
                {t("documents.stats.clients")}
              </div>
            </div>
          </div>
        </div>
  ...
  const renderDocumentPreview = (doc: any) => {
    ...
    return (
      <div
        className={`aspect-video ${isDarkMode ? "bg-black/60" : "bg-gray-200"} rounded-lg flex flex-col items-center justify-center p-4`}
      >
        <FileCheck className={`h-12 w-12 ${theme.secondaryText} mb-2`} />
        <p className="text-sm text-gray-400">
          {t("documents.preview.notAvailable").replace(
            "{ext}",
            fileExtension || "",
          )}
        </p>
  ...
  return (
    <div
      className={`min-h-screen ${theme.background} ${theme.text} p-6 transition-colors duration-200`}
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {/* Search and Filters */}
        <div
          className={`${theme.cardBg} backdrop-blur-xl border ${theme.border} rounded-xl p-4 mb-6`}
        >
          <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap">
            <div className="w-full sm:flex-1 sm:min-w-[200px]">
              <div className="relative">
                <Search
                  className={`absolute left-3 top-1/2 transform -translate-y-1/2 ${theme.secondaryText} h-4 w-4`}
                />
                <input
                  type="text"
                  value={searchTerm}
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-400">
                      {t("documents.ticket")} {doc.ticketId}
                    </p>
                    <p className="text-sm text-gray-400">
                ...
                {/* Documents list */}
                <div
                  className={`mt-4 p-3 ${theme.statsBg} border ${theme.statsBorder} rounded-lg`}
                >
                  <div className="text-sm font-medium mb-2">
                    {t("documents.uploadedDocs")}
                  </div>
      ...
      {showDetails && selectedDocument && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 md:p-6">
          <div
            className={`${theme.cardBg} border ${theme.border} rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col`}
          >
            {/* Header */}
            <div
              className={`flex items-center justify-between p-4 md:p-6 border-b ${theme.border}`}
            >
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 min-h-0">
              <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className={`text-sm ${theme.secondaryText}`}>
                        {t("documents.modal.submissionDate")}
                      </label>
                ...
                <div className="space-y-4">
                  ...
                  >
                    <h3 className="text-lg font-medium mb-3">
                      {t("documents.modal.details")}
                    </h3>
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        <span className={theme.secondaryText}>
                          {t("documents.modal.client")}
                        </span>
                        <span>{selectedDocument.submittedBy}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className={theme.secondaryText}>
                          {t("documents.modal.ticketId")}
                        </span>
                        <span>{selectedDocument.ticketId}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className={theme.secondaryText}>
                          {t("documents.modal.docType")}
                        </span>
            ...
            {/* Footer */}
            <div
              className={`border-t ${theme.border} p-4 md:p-6 flex-shrink-0`}
            >
