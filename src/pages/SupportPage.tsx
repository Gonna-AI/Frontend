...
const SupportPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.message || result.error || "Failed to create ticket",
        );
      }
  ...
  return (
    <div className="bg-[#0A0A0A] min-h-screen text-white/80 font-sans selection:bg-purple-500/30">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 h-16 bg-[#0A0A0A]/80 backdrop-blur-md border-b border-white/5">
        <div className="h-full max-w-[1600px] mx-auto px-4 md:px-8 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              to="/dashboard"
              className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white text-black text-xs font-semibold hover:bg-neutral-200 transition-colors"
            >
              Dashboard
            </Link>
            {/* Mobile Menu Toggle */}
      ...
      <div className="pt-16 max-w-[1600px] mx-auto flex min-h-[calc(100vh-64px)]">
        ...
        <aside
          className={`fixed inset-y-0 left-0 z-40 w-64 bg-[#0A0A0A] border-r border-white/5 transform transition-transform duration-300 md:translate-x-0 md:static md:block pt-20 md:pt-8 px-4 ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
        >
          <div className="space-y-8">
            <div>
              ...
              <nav className="space-y-0.5">
                <button
                  type="button"
                  className="flex items-center gap-2 px-3 py-2 rounded-lg bg-purple-500/10 text-purple-400 font-medium"
                >
            ...
            <div>
              ...
              <nav className="space-y-0.5">
                <Link
                  to="/docs#auth"
                  className="block px-3 py-1.5 text-sm text-white/50 hover:text-white transition-colors"
                >
                  Authentication
                </Link>
                <Link
          ...
          <div className="max-w-3xl">
            <div className="mb-10">
              ...
              <p className="text-lg text-white/60 leading-relaxed">
                Encountering an issue? Our engineering team is ready to help.
                Please provide detailed information to ensure a fast resolution.
              </p>
            </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-white/60 mb-6">
                      ...
                      to{" "}
                      <span className="text-white">{formData.user_email}</span>.
                    </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {PRIORITIES.map((p) => (
                        <button
                          ...
                          className={`px-2 py-2 rounded-lg text-xs font-mono font-medium border transition-all ${
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-white/30">
                    Please include any relevant error codes or request IDs.
                  </p>
                </div>
