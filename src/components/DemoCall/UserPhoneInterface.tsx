...
export default function UserPhoneInterface({
  ...
  useEffect(() => {
    if (
      ...
      const timer = setTimeout(() => {
        if (agentStatus === "idle") {
          console.log(
            "📞 Call started externally, starting recognition in overlay mode",
          );
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      console.log(
        `🎤 Initializing speech recognition for language: ${language} (${TTS_LANGUAGES[language].code})`,
      );
  ...
  const [systemChecks, setSystemChecks] = useState<
    { label: string; status: "pending" | "success" | "warning" | "error" }[]
  >([]);
      ...
      setSystemChecks((prev) => [
        ...
        prev[1],
        {
          label: elevenLabsAvailable
            ? "Voice Synthesis Active (ElevenLabs)"
            : "German Voice Unavailable (No Key)",
  ...
  const speakText = useCallback(
    (text: string) => {
      ...
      if (language === "de" && !ttsService.isElevenLabsAvailable()) {
        console.warn(
          "🇩🇪 German TTS unavailable (Missing API Key). Skipping audio.",
        );
          ...
          <div className="relative mb-12">
            ...
            <div
              className={cn(
                ...
                agentStatus === "speaking"
                  ? "animate-[spin_4s_linear_infinite] border-t-blue-500/50 border-r-transparent border-b-blue-500/50 border-l-transparent"
                  : agentStatus === "processing"
                    ? "animate-[spin_2s_linear_infinite] border-t-yellow-500/50 border-r-transparent border-b-yellow-500/50 border-l-transparent"
                    : "border-transparent",
              )}
            />
          </div>
            ...
            <AnimatePresence mode="wait">
              {(currentTranscript || lastAgentMessage) && (
                ...
                >
                  {currentTranscript ? (
                    <>
                      <p className="text-xs uppercase tracking-widest font-bold opacity-40 mb-2 text-green-400">
                        You
                      </p>
                    ...
                    <>
                      <p className="text-xs uppercase tracking-widest font-bold opacity-40 mb-2 text-blue-400">
                        Alice
                      </p>
                      <p className="text-xl font-medium leading-relaxed text-white/90">
                        {lastAgentMessage}
