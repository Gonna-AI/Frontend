...
export default function KnowledgeBase({
  ...
  const [activeTab, setActiveTab] = useState<ActiveTab>(
    (activeSection as ActiveTab) || "prompt",
  );
  ...
  const updateKnowledgeBase = (updates: Partial<KnowledgeBaseState>) => {
    setKnowledgeBase((prev) => {
      ...
      if (updates.aiVoice)
        updateContextKB({ selectedVoiceId: updates.aiVoice });
      if (updates.systemPrompt)
        updateContextKB({ systemPrompt: updates.systemPrompt });
      if (updates.persona) updateContextKB({ persona: updates.persona });
      if (updates.greeting) updateContextKB({ greeting: updates.greeting });
      if (updates.responseGuidelines)
        updateContextKB({ responseGuidelines: updates.responseGuidelines });
      if (updates.priorityRules)
        updateContextKB({ priorityRules: updates.priorityRules });
      if (updates.customInstructions)
        updateContextKB({ customInstructions: updates.customInstructions });
  ...
  const handleAddRule = () => {
    ...
    updateKnowledgeBase({
      priorityRules: [...knowledgeBase.priorityRules, newRule],
    });
    setNewRule("");
  ...
  const handleAddInstruction = () => {
    ...
    updateKnowledgeBase({
      customInstructions: [...knowledgeBase.customInstructions, newInstruction],
    });
    setNewInstruction("");
  ...
  const tabInfo = {
    prompt: {
      label: t("sidebar.systemPrompt"),
      icon: Terminal,
      description: "Define the core identity and behavior of your AI agent.",
    },
    voice: {
      label: t("sidebar.aiVoice"),
      icon: Volume2,
      description: "Choose the voice that best fits your brand personality.",
    },
    fields: {
      label: t("sidebar.contextFields"),
      icon: Database,
      description: "Structured data to extract from every conversation.",
    },
    categories: {
      label: t("sidebar.categories"),
      icon: Settings,
      description: "Call classification buckets for analytics.",
    },
    rules: {
      label: t("sidebar.priorityRules"),
      icon: AlertTriangle,
      description: "Logic for determining call priority levels.",
    },
    instructions: {
      label: t("sidebar.instructions"),
      icon: Lightbulb,
      description: "Specific behavioral guidelines for the agent.",
    },
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            title={
              localLLMConnected
                ? `${t("config.groqConnected")}: ${localLLMModel}`
                : t("config.groqOffline")
            }
            className={cn(
        ...
        >
          <VoiceSelector
            ...
            onVoiceSelect={(voiceId) =>
              updateKnowledgeBase({ aiVoice: voiceId })
            }
              ...
              >
                <div className="space-y-2">
                  <textarea
                    ...
                    onChange={(e) =>
                      updateKnowledgeBase({ systemPrompt: e.target.value })
                    }
                    rows={12}
                    className={cn(
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateKnowledgeBase({ persona: e.target.value })
                      }
                      className={cn(
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateKnowledgeBase({ greeting: e.target.value })
                      }
                      className={cn(
                        "w-full p-4 rounded-xl text-sm focus:outline-none focus:ring-1 transition-all",
                        isDark
              ...
              >
                      ...
                      >
                        <input
                          ...
                          className={cn(
                            ...
                            isDark
                              ? "border-white/10 focus:border-white/40 placeholder-white/20"
                              : "border-black/10 focus:border-black/20 placeholder-black/40",
                          )}
                        />
                        <input
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                          <button
                            ...
                            className={cn(
                              "text-xs px-3 py-1.5 rounded hover:bg-white/10",
                          ...
                          <button
                            ...
                            className={cn(
                              "text-xs font-semibold px-3 py-1.5 rounded bg-white text-black",
                            )}
                          >
                            Save Field
                          </button>
                ...
                >
                  {knowledgeBase.contextFields.map((field) => (
                    <div
                      key={field.id}
                      className="py-4 flex items-center justify-between group transition-colors px-2 rounded-lg hover:bg-white/5"
                    >
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {field.required && (
                            <span className="text-rose-400 text-xs font-medium bg-rose-400/10 px-1.5 py-0.5 rounded-full">
                              Required
                            </span>
                          )}
                          <span
                            className={cn(
                              "text-[10px] uppercase border px-1.5 py-0.5 rounded font-medium",
                              isDark
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 text-rose-400 p-2 hover:bg-rose-400/10 rounded-lg transition-all"
                      >
              ...
              >
                ...
                >
                  {knowledgeBase.categories.map((cat) => (
                    <div
                      key={cat.id}
                      className="py-4 flex items-center justify-between group px-2 rounded-lg hover:bg-white/5 transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        <div
              ...
              >
                  ...
                  >
                    ...
                    <div className="flex justify-end gap-2 pt-2">
                      ...
                      <button
                        ...
                        className="text-xs font-semibold px-3 py-1.5 rounded bg-white text-black"
                      >
                        Save Rule
                      </button>
                    </div>
                ...
                >
                  {knowledgeBase.priorityRules.map((rule, i) => (
                    <div
                      key={i}
                      className="py-4 flex items-start gap-3 group px-2 rounded-lg hover:bg-white/5 transition-colors"
                    >
                ...
                >
                  {knowledgeBase.customInstructions.map((inst, i) => (
                    <div
                      ...
                      className="py-4 flex items-start gap-3 group px-2 rounded-lg hover:bg-white/5 transition-colors"
                    >
                      <span className="w-5 h-5 rounded-full bg-purple-500/10 text-purple-400 flex items-center justify-center text-xs font-medium border border-purple-500/20">
                        {i + 1}
                      </span>
                      <p
                        className={cn(
