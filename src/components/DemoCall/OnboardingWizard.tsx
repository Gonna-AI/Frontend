...
export default function OnboardingWizard({ isDark = true, onComplete }: OnboardingWizardProps) {
    ...
    const [generatedConfig, setGeneratedConfig] = useState<Partial<KnowledgeBaseConfig> | null>(null);
    ...
    const startConversation = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch(`${SUPABASE_URL}/functions/v1/api-onboarding`, {
                method: 'POST',
                headers: {
    ...
    const generateConfig = useCallback(async (conversationMessages: ChatMessage[]) => {
        try {
            const { data: { session } } = await supabase.auth.getSession();
            const response = await fetch(`${SUPABASE_URL}/functions/v1/api-onboarding`, {
                method: 'POST',
                                    ...
                                    >
                                        ...
                                        {msg.role === 'user' && (
                                            <div className={cn(
                                                ...
                                                <User className={cn("w-4 h-4", isDark ? "text-white/60" : "text-gray-600")} />
                                            </div>
                                        )}
                                    </motion.div>
                                ))}
                            ...
                            >
                                ...
                                <div className="space-y-3 text-sm">
                                    {generatedConfig.persona && (
                                        <div>
                                            <span className={cn("font-medium", textSecondary)}>Persona: </span>
                                            <span className={textPrimary}>{generatedConfig.persona}</span>
                                        </div>
                                    )}
                                    {generatedConfig.greeting && (
                                        <div>
                                            <span className={cn("font-medium", textSecondary)}>Greeting: </span>
                                            <span className={textPrimary}>"{generatedConfig.greeting}"</span>
                                        </div>
                                    )}
                                    {generatedConfig.contextFields && generatedConfig.contextFields.length > 0 && (
                                        <div>
                                            <span className={cn("font-medium", textSecondary)}>Fields: </span>
                                            <span className={textPrimary}>{generatedConfig.contextFields.map(f => f.name).join(', ')}</span>
                                        </div>
                                    )}
                                    {generatedConfig.categories && generatedConfig.categories.length > 0 && (
                                        <div>
                                            <span className={cn("font-medium", textSecondary)}>Categories: </span>
                                    ...
                                    {generatedConfig.priorityRules && generatedConfig.priorityRules.length > 0 && (
                                        <div>
                                            <span className={cn("font-medium", textSecondary)}>Priority Rules: </span>
                                            <span className={textPrimary}>{generatedConfig.priorityRules.length} rules configured</span>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="flex gap-3 mt-5">
                                    ...
                                    >
                                        ...
                                        {isDeploying ? t('onboarding.deploying') : t('onboarding.deploy')}
                                    </button>
                                    <button
                                        onClick={() => {
