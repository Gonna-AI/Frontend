...
export function updateGroqSettings(
  settings: Partial<GroqSettings>,
): GroqSettings {
  const current = getGroqSettings();
  const updated = { ...current, ...settings };
  localStorage.setItem(getStorageKey(), JSON.stringify(updated));
  window.dispatchEvent(
    new CustomEvent("groq-settings-updated", { detail: updated }),
  );
  return updated;
}
...
export default function GroqSettingsPage({
  ...
  const saveSettings = (newSettings: GroqSettings) => {
    ...
    window.dispatchEvent(
      new CustomEvent("groq-settings-updated", { detail: newSettings }),
    );
