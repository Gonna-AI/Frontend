...
import { Plus, X, ChevronDown, Settings, Trash2, Users } from "lucide-react";
...
export default function UserSessionSwitcher({
  ...
  const createNewSession = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from("knowledge_base_config").insert({
        id: newId,
        config: newConfig,
  ...
  const getSessionColor = (id: string, isDark: boolean) => {
    const colors = [
      ...
      {
        bg: isDark ? "bg-lime-500/10" : "bg-lime-100",
        text: isDark ? "text-lime-400" : "text-lime-600",
      },
      {
        bg: isDark ? "bg-fuchsia-500/10" : "bg-fuchsia-100",
        text: isDark ? "text-fuchsia-400" : "text-fuchsia-600",
      },
      {
      ...
      {
        bg: isDark ? "bg-violet-500/10" : "bg-violet-100",
