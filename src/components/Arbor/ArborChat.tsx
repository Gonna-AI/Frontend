...
export default function ArborChat({
  mode,
  messages,
  onSendMessage,
  onClearChat,
      ...
      <div className="flex-1 overflow-y-auto space-y-3 sm:space-y-4 px-1 custom-scrollbar mb-4">
        {messages.length === 0 ? (
          ...
          messages.map((message, idx) => (
            ...
            >
              <div
                className={`max-w-[90%] sm:max-w-[80%] p-3 sm:p-4 rounded-xl sm:rounded-2xl backdrop-blur-sm ${
                  message.role === "user"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="flex justify-center mb-3 sm:mb-4">
            <div className="p-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-sm inline-flex relative">
              <div
                className={`absolute top-1.5 bottom-1.5 left-1.5 w-16 sm:w-20 rounded-full transition-all duration-300 ${getActiveColor()} ${
                  mode === "ask"
                    ? "translate-x-0"
                    : mode === "tell"
              ...
              <button
                ...
                className={`relative w-16 sm:w-20 py-2 sm:py-2.5 rounded-full text-xs sm:text-sm font-medium transition-colors duration-200 z-10 ${
                  mode === "ask"
              ...
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2 text-xs sm:text-sm">
                  ...
                  <span className="px-2 py-0.5 bg-emerald-500/20 border border-emerald-500/30 rounded-full text-emerald-400 text-xs">
                    {isReindexing
                      ? t("arbor.statusIndexing")
                      : t("arbor.statusIndexed")}
                  </span>
                </div>
