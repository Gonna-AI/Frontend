...
import { Search, Filter, Sparkles, FileText, Clock, AlertCircle } from 'lucide-react';
...
export default function ArborSearch({ isIndexed, onIndexDocuments, documentCount }: ArborSearchProps) {
                ...
                <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <label className="flex items-center gap-2 text-sm text-white/70 cursor-pointer">
                        <input
                            ...
                            onChange={(e) => setFilters({ ...filters, generateSummaries: e.target.checked })}
                        ...
                        >
                            <div className="flex items-start justify-between mb-3">
                                ...
                                <div className="flex items-center gap-2">
                                    <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getUrgencyColor(result.metadata.urgency.level)}`}>
                            ...
                            {result.snippets.length > 0 && (
                                <div className="space-y-2 mb-4">
                                    {result.snippets.map((snippet, idx) => (
                                        <p key={idx} className="text-sm text-white/60 leading-relaxed pl-8 border-l-2 border-emerald-500/30">
                                            ...{snippet}...
                                        </p>
                                    ))}
                                </div>
                            ...
                            <div className="flex items-center gap-4 text-xs text-white/40">
                                ...
                                {result.metadata.contacts.emails.length > 0 && (
                                    <span className="flex items-center gap-1">
                                        📧 {result.metadata.contacts.emails.length} contact{result.metadata.contacts.emails.length !== 1 ? 's' : ''}
                                    </span>
                                )}
            ...
            {query && results.length === 0 && !isSearching && (
                <div className="flex items-center justify-center py-16">
                    <div className="text-center space-y-3">
                        ...
                        <p className="text-white/40 text-sm">Try adjusting your search query or filters</p>
                    </div>
                </div>
            )}
        </div>
