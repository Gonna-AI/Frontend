...
export default function DeepSearchReasoning({
  query,
  onComplete,
}: DeepSearchReasoningProps) {
  const [steps, setSteps] = useState<ReasoningStep[]>([
    {
      id: "search",
      label: "Semantic Search",
      status: "pending",
      detail: "Searching through the legal database using AI embeddings...",
    },
    {
      id: "collect",
      label: "Collecting Cases",
      status: "pending",
      count: 0,
      detail: "Scanning cases to find relevant matches...",
    },
    {
      id: "graph",
      label: "Graph Analysis",
      status: "pending",
      detail: "Analyzing citation networks to find connected cases...",
    },
    {
      id: "precedents",
      label: "Precedent Chain",
      status: "pending",
      detail: "Mapping legal precedent relationships and hierarchies...",
    },
    {
      id: "synthesis",
      label: "Synthesizing Results",
      status: "pending",
      detail: "Ranking and filtering the most relevant cases...",
    },
  ]);
  ...
  useEffect(() => {
    const processSteps = async () => {
      ...
      setSteps((prev) =>
        prev.map((s) =>
          s.id === "search" ? { ...s, status: "processing" } : s,
        ),
      );
      await new Promise((resolve) => setTimeout(resolve, getRandomDelay()));
      setSteps((prev) =>
        prev.map((s) => (s.id === "search" ? { ...s, status: "complete" } : s)),
      );
        ...
        prev.map((s) =>
          s.id === "collect"
            ? {
                ...
                status: "processing",
                count: 0,
                detail: "Starting case collection...",
              }
            : s,
        ),
      );
        ...
        prev.map((s) =>
          s.id === "collect"
            ? {
                ...
                status: "complete",
                count: 13,
                detail: "Successfully collected 13 relevant cases",
              }
            : s,
        ),
      );
        ...
        prev.map((s) =>
          s.id === "graph"
            ? {
                ...s,
                status: "processing",
                detail: "Building citation graph from collected cases...",
              }
            : s,
        ),
      );
      ...
      setSteps((prev) =>
        prev.map((s) =>
          s.id === "graph"
            ? {
                ...s,
                detail: "Identifying central nodes and influential cases...",
              }
            : s,
        ),
      );
        ...
        prev.map((s) =>
          s.id === "graph"
            ? {
                ...s,
                status: "complete",
                detail: "Citation network analysis complete",
              }
            : s,
        ),
      );
        ...
        prev.map((s) =>
          s.id === "precedents"
            ? {
                ...s,
                status: "processing",
                detail: "Tracing precedent chains through case citations...",
              }
            : s,
        ),
      );
      ...
      setSteps((prev) =>
        prev.map((s) =>
          s.id === "precedents"
            ? { ...s, detail: "Evaluating precedent strength and relevance..." }
            : s,
        ),
      );
      await new Promise((resolve) => setTimeout(resolve, 1500));
        ...
        prev.map((s) =>
          s.id === "precedents"
            ? {
                ...s,
                status: "complete",
                detail: "Precedent mapping completed successfully",
              }
            : s,
        ),
      );
        ...
        prev.map((s) =>
          s.id === "synthesis"
            ? {
                ...s,
                status: "processing",
                detail: "Synthesizing findings from all analysis layers...",
              }
            : s,
        ),
      );
      ...
      setSteps((prev) =>
        prev.map((s) =>
          s.id === "synthesis"
            ? {
                ...s,
                detail: "Ranking cases by relevance and precedent strength...",
              }
            : s,
        ),
      );
        ...
        prev.map((s) =>
          s.id === "synthesis"
            ? {
                ...s,
                status: "complete",
                detail: "Analysis complete - found 2 highly relevant cases",
              }
            : s,
        ),
      );
    ...
    <div className="my-2 px-4 max-w-2xl ml-auto">
      ...
      <div className="flex items-center justify-end gap-3 mb-4 opacity-70">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-violet-300 font-medium uppercase tracking-wide">
            Deep Search
          </span>
        </div>
        ...
        <div className="space-y-6">
          {steps.map((step, idx) => (
            <div key={step.id} className="relative flex gap-4">
              {/* Dot Indicator */}
              <div
                className={`relative z-10 flex-none w-3.5 h-3.5 rounded-full border-2 transition-all duration-500 bg-[#0a0a0a] ${
