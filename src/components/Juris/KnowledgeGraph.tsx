...
export default function KnowledgeGraph({ caseData }: KnowledgeGraphProps) {
  ...
  useEffect(() => {
    ...
    // Add statute nodes
    caseData.metadata.statutes_cited
      .slice(0, 3)
      .forEach((statute: string, idx: number) => {
        const node: Node = {
          id: `statute-${idx}`,
          type: "statute",
  ...
  const getNodeColor = (type: string) => {
    switch (type) {
      case "case":
        return { bg: "#7c3aed", border: "#a78bfa", text: "#fff" };
      case "statute":
        return { bg: "#6366f1", border: "#818cf8", text: "#fff" };
      case "case_type":
        return { bg: "#8b5cf6", border: "#a78bfa", text: "#fff" };
      case "court":
        return { bg: "#6b21a8", border: "#9333ea", text: "#fff" };
      case "judge":
        return { bg: "#7e22ce", border: "#a855f7", text: "#fff" };
      case "concept":
        return { bg: "#581c87", border: "#7e22ce", text: "#fff" };
      default:
        return { bg: "#6b21a8", border: "#9333ea", text: "#fff" };
    }
  };
        ...
        {nodes.map((node) => {
          ...
          return (
            <g
              ...
              style={{ cursor: "grab" }}
              className={
                draggedNodeRef.current?.id === node.id ? "cursor-grabbing" : ""
              }
            >
              {/* Glow effect */}
              <circle
