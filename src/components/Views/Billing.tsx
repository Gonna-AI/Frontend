...
export default function Billing() {
  ...
  const renderUsageBar = (used, total) => {
    const percentage =
      typeof total === "number" ? Math.min((used / total) * 100, 100) : 50;
    return (
  ...
  const GlassContainer = ({ children, className }) => (
    <div
      className={`relative overflow-hidden rounded-xl p-6 bg-white/10 dark:bg-black/20 border border-black/10 dark:border-white/10 transition-all duration-200 ${className}`}
    >
      {children}
    </div>
  );
        ...
        {plans.map((plan) => (
          <GlassContainer
            ...
            className={`transition-transform duration-200 hover:scale-[1.02] ${
              plan.recommended &&
              "ring-2 ring-purple-600/50 dark:ring-purple-400/50"
            }`}
          >
            {plan.recommended && (
