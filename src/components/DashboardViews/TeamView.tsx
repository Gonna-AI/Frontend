...
export default function TeamView({ isDark = true }: { isDark?: boolean }) {
        ...
        setMembers(
          result.members.map((m: any) => ({
            ...
            lastActive: m.last_active
              ? getRelativeTime(new Date(m.last_active))
              : undefined,
          })),
        );
      }
  ...
  const getRelativeTime = (date: Date) => {
    ...
    if (minutes < 60)
      return t("team.minAgo").replace("{count}", minutes.toString());
    const hours = Math.floor(minutes / 60);
    if (hours < 24)
      return t("team.hoursAgo").replace("{count}", hours.toString());
    return t("team.daysAgo").replace(
      "{count}",
      Math.floor(hours / 24).toString(),
    );
  };
