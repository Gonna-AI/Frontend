    ...
    >
      ...
      <div className="z-10 mt-auto">
        ...
        {progress !== undefined && (
          <div
            className={cn(
              "w-full h-1 rounded-full mt-3 mb-2",
...
export default function UsageView({ isDark = true }: { isDark?: boolean }) {
  ...
  const fetchLiveBalance = async () => {
    try {
      ...
      const res = await fetch(
        `https://xlzwfkgurrrspcdyqele.supabase.co/functions/v1/api-billing/balance`,
        {
  ...
  const handleRedeem = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `https://xlzwfkgurrrspcdyqele.supabase.co/functions/v1/api-billing/redeem`,
        {
          method: "POST",
          headers: {
      ...
      setRedeemMessage({
        type: "success",
        text: t("usage.redeemSuccess").replace(
          "{count}",
          data.added_amount.toString(),
        ),
      });
      setRedeemCode("");
      fetchLiveBalance();
      ...
      setRedeemMessage({
        type: "error",
        text: e.message || t("usage.redeemError"),
      });
    } finally {
  ...
  const usageStats = useMemo(() => {
    ...
    const textCreditsUsed =
      Math.ceil(totalTextRequests / 10) * TEXT_CREDITS_PER_10_REQUESTS;
    const totalCreditsUsed = voiceCreditsUsed + textCreditsUsed;
    const fallbackCreditsRemaining = Math.max(
      0,
      TOTAL_CREDITS - totalCreditsUsed,
    );

    const creditsRemaining =
      liveCredits !== null ? liveCredits : fallbackCreditsRemaining;
    ...
    return {
      ...
      creditsRemaining,
      creditsUsedPercent:
        liveCredits !== null ? 100 : (totalCreditsUsed / TOTAL_CREDITS) * 100,
    };
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const voiceCount = callHistory.filter(
        (c) =>
          c.type === "voice" &&
      ...
      const textCount = callHistory.filter(
        (c) =>
          c.type === "text" &&
          new Date(c.date) >= dayStart &&
          new Date(c.date) <= dayEnd,
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className={isDark ? "text-gray-300" : "text-gray-600"}>
                {t("usage.voice")}: {payload[0].value}
              </span>
            </div>
            <div className="flex items-center gap-2 text-xs">
              <div className="w-2 h-2 rounded-full bg-blue-500" />
              <span className={isDark ? "text-gray-300" : "text-gray-600"}>
                {t("usage.text")}: {payload[1].value}
              </span>
            </div>
          </div>
        </div>
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto pb-10">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              >
                {t("usage.creditsRemaining").replace(
                  "{count}",
                  String(usageStats.creditsRemaining),
                )}
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatsCard
          ...
          change={
            analytics.followUpRequired > 0
              ? t("usage.followUps").replace(
                  "{count}",
                  String(analytics.followUpRequired),
                )
              : undefined
          }
          trend="neutral"
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-white/5">
              ...
              >
                ...
                >
                  {t("usage.mins").replace(
                    "{count}",
                    String(usageStats.totalVoiceMinutes),
                  )}
                </td>
                <td
                  className={cn(
                ...
                >
                  {t("usage.credits").replace("{count}", "50")}
                </td>
              </tr>
              <tr
                className={cn(isDark ? "hover:bg-white/5" : "hover:bg-gray-50")}
              >
                ...
                >
                  {t("usage.reqs").replace(
                    "{count}",
                    String(usageStats.totalTextRequests),
                  )}
                </td>
                <td
                  className={cn(
