...
function useRazorpay() {
  ...
  return {
    ...
    isLoading,
    Razorpay:
      scriptLoadedRef.current || (!isLoading && !error) ? RazorpayClass : null,
  };
}
...
export default function BillingView({ isDark = true }: { isDark?: boolean }) {
  ...
  const fetchSubscriptionInfo = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${supabaseUrl}/functions/v1/api-billing/balance`,
        {
  ...
  const usageStats = useMemo(() => {
    ...
    const totalVoiceSeconds = voiceCalls.reduce(
      (acc, call) => acc + (call.duration || 0),
      0,
    );
    const totalVoiceMinutes = Math.ceil(totalVoiceSeconds / 60);
    const totalTextRequests = textChats.reduce(
      (acc, call) =>
        acc + call.messages.filter((m) => m.speaker === "user").length,
      0,
    );
    ...
    const creditsRemaining = subInfo
      ? creditsBalance
      : Math.max(0, 50 - totalCreditsUsed);
  ...
  const nextResetDate = useMemo(() => {
    ...
    return nextMonth.toLocaleDateString(
      navigator.language === "de-DE" ? "de-DE" : "en-US",
  ...
  const handleCheckout = async (planName: string) => {
    ...
    try {
      ...
      if (error || !Razorpay) {
        setPaymentError(
          t("billing.error.gatewayFailed").replace(
            "{error}",
            error || "Unknown error",
          ),
        );
      ...
      const res = await fetch(
        `${supabaseUrl}/functions/v1/api-billing/create-order`,
        {
          method: "POST",
          headers: {
        ...
        handler: async function (response: RazorpayPaymentSuccessResponse) {
          try {
            const verifyRes = await fetch(
              `${supabaseUrl}/functions/v1/api-billing/verify-payment`,
              {
                method: "POST",
                headers: {
                  Authorization: `Bearer ${session.access_token}`,
            ...
            if (verifyRes.ok && verifyData.success) {
              ...
              throw new Error(
                verifyData.error || t("billing.error.signatureFailed"),
              );
            }
          } catch (err: unknown) {
            setPaymentError(
              getErrorMessage(err, t("billing.error.verificationFailed")),
            );
            setPaymentStatus("failed");
          } finally {
            setIsProcessing(false);
      ...
      setPaymentError(
        getErrorMessage(error, "An unexpected payment error occurred."),
      );
      setPaymentStatus("failed");
      setIsProcessing(false);
    }
  ...
  const plans: Plan[] = [
    {
      ...
      features: [
        {
          text: t("billing.features.free.credits"),
          included: true,
          highlight: true,
        },
    ...
    },
    {
      ...
      features: [
        {
          text: t("billing.features.pro.credits"),
          included: true,
          highlight: true,
        },
        {
          text: t("billing.features.pro.voice"),
          included: true,
          highlight: true,
        },
    ...
    },
    {
      ...
      features: [
        {
          text: t("billing.features.enterprise.credits"),
          included: true,
          highlight: true,
        },
        { text: t("billing.features.enterprise.gpu"), included: true },
        {
          text: t("billing.features.enterprise.unlimitedVoices"),
          included: true,
        },
    ...
    <div className="space-y-8 max-w-[1600px] mx-auto pb-10">
            ...
            >
              <div
                className={cn(
                  ...
                  usageStats.creditsUsedPercent >= 100
                    ? "bg-rose-500"
                    : "bg-emerald-500",
                )}
                style={{
                  width: `${Math.min(Math.max(usageStats.creditsUsedPercent, 0), 100)}%`,
                }}
              />
            </div>
        ...
        {plans.map((plan) => (
          <div
            ...
            className={cn(
              ...
              plan.popular
                ? isDark
                  ? "bg-gradient-to-b from-[#09090B] to-emerald-950/20 border-emerald-500/50 shadow-[0_0_40px_-10px_rgba(16,185,129,0.2)]"
          ...
          >
            ...
            <button
              ...
              onClick={() => {
                if (plan.name === "Pro" && !plan.current)
                  handleCheckout(plan.name);
                else if (plan.name === "Enterprise" && !plan.current)
                  window.location.href = "mailto:sales@clerktree.com";
              }}
              className={cn(
                ...
                plan.current
                  ...
                  : plan.popular
                    ...
                    : isDark
                      ? "bg-white text-black hover:bg-gray-200 transform hover:-translate-y-0.5"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12">
        ...
        >
          ...
          <button
            onClick={() =>
              (window.location.href = "mailto:enterprise@clerktree.com")
            }
            className={cn(
              "w-full py-3 rounded-xl text-sm font-medium border transition-colors",
              isDark
                ? "border-white/10 hover:bg-white/5 text-white"
                : "border-black/10 hover:bg-gray-50 text-black",
            )}
          >
