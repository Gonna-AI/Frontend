    ...
    const renderMonth = (offset: number) => {
        ...
        return (
            <div className="w-[300px] p-2">
                <div className={cn("text-center font-medium mb-4", isDark ? "text-white" : "text-black")}>
                    {formatMonthYear(d)}
                </div>
                <div className="grid grid-cols-7 gap-1 mb-2">
                    {weekDays.map(wd => (
                        <div key={wd} className={cn("h-8 flex items-center justify-center text-xs text-white/40", isDark ? "text-white/40" : "text-black/40")}>
                            {wd}
                        </div>
                    ))}
                </div>
                        ...
                        return (
                            <button
                                ...
                                className={cn(
                                    ...
                                    !isDark && !isSel && !isRange && "text-black hover:bg-black/5",
            ...
            {isOpen && (
                <div className={cn(
                    ...
                    <div className="flex items-center justify-between px-2">
                        <button onClick={prevMonth} className="p-1 hover:bg-white/10 rounded-md">
                            <ChevronLeft className={cn("w-5 h-5", isDark ? "text-white" : "text-black")} />
                        </button>
                        <button onClick={nextMonth} className="p-1 hover:bg-white/10 rounded-md">
