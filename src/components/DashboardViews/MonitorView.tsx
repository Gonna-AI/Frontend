...
export default function MonitorView({ isDark = true }: { isDark?: boolean }) {
  ...
  const [activeSection, setActiveSection] = useState<"history" | "live">(
    "live",
  );
  const [chartRange, setChartRange] = useState<"week" | "month" | "all">(
    "month",
  );
  ...
  const filteredHistory = useMemo(() => {
    return callHistory.filter(
      (call) => filterPriority === "all" || call.priority === filterPriority,
    );
  ...
  const chartData = useMemo(() => {
    ...
    if (chartRange === "week" || chartRange === "month") {
      ...
      for (let i = daysCount - 1; i >= 0; i--) {
        ...
        const callCount = callHistory.filter(
          (c) => new Date(c.date) >= dayStart && new Date(c.date) <= dayEnd,
        ).length;
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const callCount = callHistory.filter(
          (c) => new Date(c.date) >= monthStart && new Date(c.date) <= monthEnd,
        ).length;
        ...
        <StatsCard
          ...
          progress={
            analytics.totalCalls > 0
              ? (analytics.followUpRequired / analytics.totalCalls) * 100
              : 0
          }
          isDark={isDark}
        />{" "}
        ...
        <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 sm:gap-0">
          ...
          >
            {(["week", "month", "all"] as const).map((range) => (
              ...
              >
                {range === "week"
                  ? t("monitor.filter.week") || "Week"
                  : range === "month"
                    ? t("monitor.filter.month") || "Month"
                    : t("monitor.filter.allTime") || "All Time"}
              </button>
            ))}
          </div>
        </div>
          ...
          >
            ...
            >
              <Radio
                className={cn(
                  "w-4 h-4",
                  activeSection === "live" && "text-rose-500 animate-pulse",
                )}
              />
              {t("monitor.tab.live")}
            </SectionTab>
                  ...
                  >
                    <tr>
                      ...
                      <th className="px-6 py-5">
                        {t("monitor.table.category")}
                      </th>
                      <th className="px-6 py-5">{t("monitor.table.status")}</th>
                      <th className="px-6 py-5">
                        {t("monitor.table.duration")}
                      </th>
                        ...
                        <React.Fragment key={call.id}>
                          ...
                          >
                            ...
                            <td className="px-6 py-5">
                              ...
                              >
                                {call.category?.name ||
                                  t("monitor.category.uncategorized")}
                              </span>
                            </td>
                            <td className="px-6 py-5">
                              <span
                              ...
                              >
                                <div
                                  className={cn(
                                    ...
                                    call.priority === "critical"
                                      ? "bg-red-400 font-bold animate-pulse"
                                      : call.priority === "high"
                                        ? "bg-orange-400"
                                        : "bg-emerald-400",
                                  )}
                                />
                                {call.priority === "critical"
                                  ? t("monitor.status.critical")
                                  : call.priority === "high"
                                    ? t("monitor.status.high")
                                    : t("monitor.status.resolved")}
                              </span>
                            </td>
                            <td
                              className={cn(
                                "px-6 py-5 font-mono text-xs",
                            ...
                            <td className="px-6 py-5 text-right">
                              <button
                                onClick={() =>
                                  setExpandedLogId(
                                    expandedLogId === call.id ? null : call.id,
                                  )
                                }
                                className={cn(
                                  ...
                                  expandedLogId === call.id
                                    ...
                                    : isDark
                                      ...
                                      : "opacity-0 group-hover:opacity-100 hover:bg-gray-100 text-gray-400 hover:text-gray-900",
                                )}
                              >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                  ...
                                  {call.extractedFields.length > 0 && (
                                    <div>
                                      ...
                                      >
                                        {t("monitor.detail.extractedData")}
                                      </h4>
                                      <div
                                        className={cn(
                                        ...
                                        <div className="space-y-1 p-3">
                                          {call.extractedFields.map((field) => (
                                            <div
                                              key={field.id}
                                              className={cn(
                                                "flex items-center justify-between text-sm py-1.5 px-2 rounded hover:bg-black/5 dark:hover:bg-white/5 transition-colors",
                                              )}
                                            >
                                              <span
                                                className={cn(
