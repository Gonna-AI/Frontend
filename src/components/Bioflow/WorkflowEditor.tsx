...
const WorkflowEditorCore = ({
  ...
  const runSimulation = useCallback(async () => {
    ...
    for (const stepNodes of useCase.trace) {
      setNodes((nds) =>
        nds.map((n) =>
          stepNodes.includes(n.id)
            ? { ...n, data: { ...n.data, status: NodeStatus.PROCESSING } }
            : n,
        ),
      );
      setEdges((eds) =>
        eds.map((e) =>
          stepNodes.includes(e.target)
            ? { ...e, style: { ...e.style, stroke: "#fbbf24", strokeWidth: 2 } }
            : e,
        ),
      );
      ...
      setNodes((nds) =>
        nds.map((n) =>
          stepNodes.includes(n.id)
            ? { ...n, data: { ...n.data, status: NodeStatus.COMPLETED } }
            : n,
        ),
      );
      setEdges((eds) =>
        eds.map((e) =>
          stepNodes.includes(e.target)
            ? {
                ...e,
                animated: false,
                style: { ...e.style, stroke: "#10b981", strokeWidth: 1.5 },
              }
            : e,
        ),
      );
...
const WorkflowEditor = ({
  initialUseCase = "doctor",
  showInspector = true,
}: WorkflowEditorProps) => {
  const [selectedUseCase, setSelectedUseCase] =
    useState<UseCaseId>(initialUseCase);
