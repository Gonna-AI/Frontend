...
const GlassNode = ({ data, selected }: NodeProps<NodeData>) => {
  ...
  const getStatusInfo = () => {
    if (isProcessing)
      return { color: "bg-yellow-400 animate-pulse", text: "Processing..." };
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-white/5 bg-white/[0.02] rounded-t-xl">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-[10px] font-medium uppercase tracking-wider ${scheme.text}`}
          >
            {scheme.label}
          </span>
        </div>
        <span className="text-[10px] text-slate-500">{statusInfo.text}</span>
      </div>
            ...
            {data.metadata?.confidence && (
              <div className="flex items-center gap-1.5">
                <span className="text-[10px] text-emerald-400">
                  {Math.round(data.metadata.confidence * 100)}% sure
                </span>
              </div>
            )}
          </div>
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-black/40 border-t border-white/5 rounded-b-xl">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[9px] text-slate-500">
            Receives: {data.inputType || "Start"}
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="text-[9px] text-slate-500">
            Outputs: {data.outputType || "Done"}
          </span>
          <div
            className={`w-1 h-1 rounded-full ${isCompleted ? "bg-emerald-500" : "bg-slate-600"}`}
          />
        </div>
      </div>
...
function arePropsEqual(
  ...
  return (
    ...
    prevProps.data.metadata?.executionTime ===
      nextProps.data.metadata?.executionTime &&
