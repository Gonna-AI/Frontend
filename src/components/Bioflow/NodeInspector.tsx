...
import { X, Activity, Database, MessageSquare, CheckCircle2 } from 'lucide-react';
...
export const NodeInspector: React.FC<NodeInspectorProps> = ({ node, onClose, isMobile = false }) => {
    if (!node) {
        // Desktop empty state
        if (!isMobile) {
            return (
    ...
    const getCategoryLabel = (category: string) => {
        switch (category) {
            case 'trigger': return 'Starting Point';
            case 'agent': return 'AI Processing';
            case 'tool': return 'External Service';
            case 'action': return 'Final Action';
            default: return 'Step';
        }
    };

    const getStatusInfo = () => {
        if (node.data.status === NodeStatus.PROCESSING) return { text: 'Processing...', color: 'text-yellow-400' };
        if (node.data.status === NodeStatus.COMPLETED) return { text: 'Completed', color: 'text-emerald-400' };
        if (node.data.status === NodeStatus.ERROR) return { text: 'Error', color: 'text-red-400' };
                ...
                <div className="p-3 border-b border-white/10 flex items-center justify-between bg-black/30 shrink-0">
                    <div className="flex items-center gap-2">
                        <div className="p-1.5 rounded-lg bg-indigo-500/10 border border-indigo-500/20">
                            <DynamicIcon name={node.data.icon} className="w-4 h-4 text-indigo-400" />
                        </div>
                        <div>
                            <h2 className="text-sm font-bold text-white">{node.data.label}</h2>
                            <div className="flex items-center gap-2">
                                <span className="text-[10px] text-slate-400">{getCategoryLabel(node.data.category)}</span>
                                <span className={`text-[10px] font-medium ${statusInfo.color}`}>{statusInfo.text}</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-3 space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <div className="bg-slate-800/30 p-2 rounded-lg border border-white/5 text-center">
                            ...
                            <div className="text-[10px] text-white font-medium truncate">{node.data.metadata?.model || 'N/A'}</div>
                        </div>
                        <div className="bg-slate-800/30 p-2 rounded-lg border border-white/5 text-center">
                            <div className="text-[9px] text-slate-500 uppercase">Speed</div>
                            <div className="text-[10px] text-white font-medium">{node.data.metadata?.executionTime || '-'}</div>
                        </div>
                        <div className="bg-slate-800/30 p-2 rounded-lg border border-white/5 text-center">
                            <div className="text-[9px] text-slate-500 uppercase">Confidence</div>
                            <div className="text-[10px] text-white font-medium">
                                {node.data.metadata?.confidence ? `${(node.data.metadata.confidence * 100).toFixed(0)}%` : '-'}
                            </div>
                        </div>
                    </div>
                </div>
    ...
    return (
        <div className="w-72 h-full bg-[#0a0a0a]/95 backdrop-blur-xl border-l border-white/10 flex flex-col z-20 transition-all">
            ...
            <div className="p-3 border-b border-white/10 flex items-start justify-between bg-black/20">
                ...
                <button onClick={onClose} className="text-slate-500 hover:text-white transition-colors p-1 hover:bg-white/10 rounded-lg">
                    <X className="w-4 h-4" />
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin">
                ...
                <div className="p-3 border-b border-white/5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <MetricCard label="Model" value={node.data.metadata?.model || 'N/A'} />
                        <MetricCard label="Confidence" value={node.data.metadata?.confidence ? `${(node.data.metadata.confidence * 100).toFixed(0)}%` : '-'} />
                        <MetricCard label="Speed" value={node.data.metadata?.executionTime || '-'} />
                        <MetricCard label="Cost" value={node.data.metadata?.cost || 'Free'} />
                    </div>
                </div>
                ...
                <div className="p-3">
                    ...
                    <div className="space-y-2">
                        <div>
                            <span className="text-[9px] text-slate-500 mb-1 block uppercase tracking-wider">Input</span>
                        ...
                        {node.data.outputs && Object.keys(node.data.outputs).length > 0 && (
                            <div>
                                <span className="text-[9px] text-slate-500 mb-1 block uppercase tracking-wider">Output</span>
...
const MetricCard = ({ label, value }: { label: string, value: string | number }) => (
    <div className="bg-slate-800/30 p-2 rounded-lg border border-white/5">
        ...
        <div className="text-[10px] text-white font-medium mt-0.5 truncate">{value}</div>
    </div>
);
