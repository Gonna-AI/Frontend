...
export default function ChatInterface({
  ...
  const handleStopAI = () => {
    ...
    setMessages((prev) => [
      ...prev,
      {
        text: "AI response stopped. You can ask another question.",
        isUser: false,
      },
    ]);
  };
  ...
  const validateImageFile = (file: File) => {
    ...
    if (!validTypes.includes(file.type)) {
      throw new Error(
        "Please upload only image files (JPEG, PNG, GIF, or WebP)",
      );
    }
      ...
      const errorMessage: Message = {
        text:
          error instanceof Error
            ? error.message
            : "Sorry, I encountered an error uploading the image.",
        isUser: false,
      };
      setMessages((prev) => [...prev, errorMessage]);
    } finally {
    ...
    <div className="flex flex-col h-full relative">
      ...
      >
        ...
        >
          ...
          {isMenuOpen && (
            <div
              className="absolute bottom-full right-0 mb-2 p-2 rounded-xl flex flex-col gap-2 md:hidden
