...
export default function CallWindow({ isDark, onClose, onStopAI, onFileUpload, ticketCode }: CallWindowProps) {
  ...
  const [callDuration, setCallDuration] = useState(() => {
    const savedDuration = localStorage.getItem('callDuration');
  ...
  useEffect(() => {
    localStorage.setItem('callDuration', JSON.stringify(callDuration));
  }, [callDuration]);
  ...
  return (
    <>
      <button onClick={() => setIsOpen(true)} className="open-call-window-button">
        Open Call Window
      </button>
